...
ices_rect <- read_csv(
  file.path("data", "raw", "fish_parameters.csv"),
...
spras <-
  ...
  filter(month(collection_date) == 05, collection_method == "Trawl") |>
  extract(
    sample_name,
    into = c("trawl_id", "sample_ID", "barcode"),
    regex = "([^.]*)\\.(.*)_(.*)",
    remove = FALSE
  ) |>
  right_join(
    read_csv(
      file.path("data", "processed", "trawl_summary.csv"),
      show_col_types = FALSE
    ),
  ...
  separate(
    lat_lon,
    into = c("sample_latitude_dd", "sample_longitude_dd"),
    sep = " "
  ) |>
  mutate(
    across(c(sample_latitude_dd, sample_longitude_dd), as.numeric),
...
smhi_shp <-
  smhi_stations |>
  st_as_sf(coords = c("sample_longitude_dd", "sample_latitude_dd"), crs = 4326)
