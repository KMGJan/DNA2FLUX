...
df_COI <-
  # COI metadata
  read_csv(
    file.path("data", "raw", "fish_coi_metadata.csv"),
    show_col_types = FALSE
  ) |>
  # The column sample_name contains multiple information
  extract(
    sample_name,
    into = c("station_ID", "sample_ID", "barcode"),
    regex = "([^.]*)\\.(.*)_(.*)",
    remove = FALSE
  ) |>
  full_join(
    ...
    read_csv(
      file.path("data", "raw", "fish_coi_asv.csv"),
      show_col_types = FALSE
    ) |>
      pivot_longer(
...
df_18S <-
  read_csv(
    file.path("data", "raw", "fish_18s_metadata.csv"),
    show_col_types = FALSE
  ) |>
  # The column sample_name contains multiple information
  extract(
  ...
  full_join(
    # Combine ASV table with metadata
    read_csv(
      file.path("data", "raw", "fish_18s_asv.csv"),
      show_col_types = FALSE
    ) |>
      pivot_longer(
        cols = where(is.numeric), # Pivot  numeric columns (abundance values)
  ...
  mutate(
    Taxa = case_when(
      ...
      ASV_ID %in%
        c(
          "94ebe204e11f85a07820a7efef4f32d5",
          "d0cd100a81e83b5d909b9001096f202f"
        ) ~
        "Other",
      TRUE ~ "Other" # Default case
    )
...
df_16S <-
  read_csv(
    file.path("data", "raw", "zooplankton_16s_metadata.csv"),
    show_col_types = FALSE
  ) |>
  full_join(
    # Combine ASV table with metadata
    read_csv(
      file.path("data", "raw", "zooplankton_16s_asv.csv"),
      show_col_types = FALSE
    ) |>
      pivot_longer(
        cols = where(is.numeric), # Pivot  numeric columns (abundance values)
  ...
  mutate(
    Taxa = case_when(
      ...
      Species %in%
        c("Nostocaceaex", "Aphanizomenon_NIES81", "Aphanizomenon_MDT14a") ~
        "Aphanizomenonaceae",
      Species %in% c("Cyanobium_PCC-6307", "Synechococcus_CC9902") ~
        "Cyanobiaceae",
...
full_combined <-
  ...
  mutate(
    sample_week = floor_date(collection_date, unit = "week", week_start = 1)
  )
...
predator_selectivity <-
  ...
  mutate(
    rra_gut = rra_gut / sum(rra_gut, na.rm = T),
  ...
  summarise(
    rra_gut = sum(rra_gut, na.rm = T),
    rra_env = sum(rra_env, na.rm = T),
    .groups = "drop_last"
  ) |>
  summarise(
    rra_gut = mean(rra_gut, na.rm = T),
    rra_env = mean(rra_env, na.rm = T),
    .groups = "drop"
  ) |>
  mutate(
    trawl_id = ifelse(
      trawl_id %in% c("By31", "By2", "By5", "By16", "By15"),
      NA,
      trawl_id
    )
  )
