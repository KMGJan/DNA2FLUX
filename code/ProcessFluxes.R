...
plan(multisession)
timeseries_fluxes <- tibble(sample_week = floor_date(date(dates), unit = "week", week_start = 1)) |>
...
# Save as csv    
write_csv(edges_df, file = file.path("data", "analyses", "timeseries_fluxes.csv"))
...
station_fluxes <- files |>
  ...
  ungroup()
# Save as csv  
write_csv(station_fluxes, file = file.path("data", "analyses", "station_fluxes.csv"))
