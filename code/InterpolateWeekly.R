...
# Read and filter
picoplankton <-
  read_csv(file.path("data", "processed", "shark", "picoplankton.csv"), show_col_types = FALSE) |> 
...
order_pp <- node_names |>
  ...
  right_join(read_csv(file.path("data", "processed", "shark", "phytoplankton.csv"), show_col_types = FALSE), 
             by = "taxon_order") |> 
...
herring <-
  list.files(file.path("data", "raw", "BIAS Herr Sprat"), pattern = "^biohe", full.names = TRUE) |> 
  readFishBiomass() |>
  # So we have everything in grams
  mutate(N = N * 1e6, # Abundance was in million -> now in individual
         biom = biom * 1e6) |> 
...
sprat <-
  list.files(file.path("data", "raw", "BIAS Herr Sprat"), pattern = "^biosp", full.names = TRUE) |> 
  readFishBiomass() |> 
  # So we have everything in grams
  mutate(N = N * 1e6, # Abundance was in million -> now in individual
         biom = biom * 1e6) |> 
...
area_df <-
  data.frame(
    ICES_rect = c("42G6", "42G7","43G6","43G7","43G8","44G6","44G7", "44G8","45G6","45G7","45G8","46G6","46G7","46G8","47G8","48G8"),
    Area_nm2 = c(266.0,986.9,269.8,913.8,106.1,200.9,960.5,456.6,72.9,908.7,947.2,38.9,452.6,884.8,264.3,53.8)) |> 
...
fish <-
  ...
  mutate(biomass = biomass / Area_m2,
         abundance = N / Area_m2) |> 
  select(node_name, ICES_rect, abundance, biomass, bodymass, year) |> 
  arrange(node_name, year, ICES_rect) |> 
  mutate(station_name = ifelse(ICES_rect %in% c("43G9", "43H0"), "BY15 GOTLANDSDJ",
                               ifelse(ICES_rect %in% c("39G5", "39G6"), "BY5 BORNHOLMSDJ",
...
fish |> 
  ...
  summarise(biomass = mean(biomass, na.rm = T),
            ...
            .groups = "drop") |> 
  filter(station_name %in% c("BY31 LANDSORTSDJ", "BY5 BORNHOLMSDJ", "BY15 GOTLANDSDJ")) |> 
  full_join(
    tibble(
      sample_week = read_csv(file.path("data", "processed", "interpolation", "zooplankton_biomass.csv"), show_col_types = FALSE) |> pull(sample_week) |> unique()) |> 
      mutate(year = year(sample_week)),
    by = "year",
...
fish |> 
  ...
  full_join(
    tibble(
      sample_week = read_csv(file.path("data", "processed", "interpolation", "zooplankton_bodymass.csv"), show_col_types = FALSE) |> pull(sample_week) |> unique()) |> 
      mutate(year = year(sample_week)),
    by = "year",
    relationship = "many-to-many") |>
...
read_csv(file.path("data", "processed", "interpolation", "fish_biomass.csv"), show_col_types = FALSE) |> 
  bind_rows(read_csv(file.path("data", "processed", "interpolation", "zooplankton_biomass.csv"), show_col_types = FALSE)) |>
  bind_rows(read_csv(file.path("data", "processed", "interpolation", "phytoplankton_biomass.csv"), show_col_types = FALSE)) |>
  write_csv(file = file.path("data", "processed", "interpolation", "weekly_biomasses.csv"))

# Merge Bodymass ----------

read_csv(file.path("data", "processed", "interpolation", "fish_bodymass.csv"), show_col_types = FALSE) |> 
  bind_rows(read_csv(file.path("data", "processed", "interpolation", "zooplankton_bodymass.csv"), show_col_types = FALSE)) |>
